...
-- Only create when both user_punch_cards and punch_card_programs exist (may be created outside this set).
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'user_punch_cards')
     AND EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'punch_card_programs') THEN
    EXECUTE $exec$
