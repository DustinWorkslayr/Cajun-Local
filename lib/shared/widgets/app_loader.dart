...
class AppLoader extends StatelessWidget {
  const AppLoader({super.key, this.color, this.size = 24, this.strokeWidth = 2}) : _fullPage = false;

  /// Full-page loader: centered, larger size. Use when the whole body is loading.
  const AppLoader.page({super.key, this.color, this.size = 48, this.strokeWidth = 3}) : _fullPage = true;
  ...
  Widget build(BuildContext context) {
    ...
    final indicator = SizedBox(
      width: size,
      height: size,
      child: CircularProgressIndicator(strokeWidth: strokeWidth, color: effectiveColor),
    );
    if (_fullPage) {
