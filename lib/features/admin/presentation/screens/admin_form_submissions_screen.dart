...
class AdminFormSubmissionsScreen extends ConsumerStatefulWidget {
  const AdminFormSubmissionsScreen({super.key, this.embeddedInShell = false, this.businessId});

  final bool embeddedInShell;
  final String? businessId;

  @override
  ConsumerState<AdminFormSubmissionsScreen> createState() => _AdminFormSubmissionsScreenState();
}

class _AdminFormSubmissionsScreenState extends ConsumerState<AdminFormSubmissionsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: FutureBuilder<List<FormSubmission>>(
        future: repo.listForAdmin(businessId: _businessIdFilter, limit: 100),
        builder: (context, snapshot) {
          ...
          if (list.isEmpty) {
            return Center(
              child: Text(
                _businessIdFilter != null ? 'No submissions for this business.' : 'No form submissions.',
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final dateStr = s.createdAt != null
                  ...
                  : '—';
              final subtitle =
                  '${s.businessName ?? s.businessId} · $templateName · $dateStr'
                  '${s.isRead ? '' : ' · Unread'}';
              return Padding(
                padding: const EdgeInsets.only(bottom: 10),
                child: AdminListCard(
                  title: s.adminNote != null && s.adminNote!.length > 40
                      ? '${s.adminNote!.substring(0, 40)}…'
                      : (s.adminNote ?? templateName),
                  subtitle: subtitle,
                  onTap: () => _openDetail(context, s),
                  badges: [if (!s.isRead) const AdminBadgeData('Unread', color: AppTheme.specRed)],
                ),
              );
            },
          );
...
class AdminFormSubmissionDetailSlideOut extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<AdminFormSubmissionDetailSlideOut> createState() => _AdminFormSubmissionDetailSlideOutState();
}

class _AdminFormSubmissionDetailSlideOutState extends ConsumerState<AdminFormSubmissionDetailSlideOut> {
  ...
  void initState() {
    ...
    _noteController = TextEditingController(text: widget.submission.adminNote ?? '');
  }
  ...
  Widget build(BuildContext context) {
    ...
    final templateName = ContactFormTemplates.getByKey(_submission.template)?.name ?? _submission.template;
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 4),
                Text(templateName, style: theme.textTheme.bodyMedium?.copyWith(color: sub)),
                const SizedBox(height: 12),
                Text(
                  'Submitted data',
                  style: theme.textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w700, color: nav),
                ...
                Container(
                  ...
                  child: _submission.data.isEmpty
                      ? Text('No data', style: theme.textTheme.bodySmall?.copyWith(color: sub))
                      : Column(
                          ...
                          children: _submission.data.entries
                              .map(
                                (e) => Padding(
                                  ...
                                  child: Text(
                                    '${e.key}: ${e.value}',
                                    style: theme.textTheme.bodySmall?.copyWith(color: nav),
                                  ),
                                ),
                              )
                              .toList(),
                ...
                if (_message != null) ...[
                  ...
                  Text(
                    ...
                    style: theme.textTheme.bodySmall?.copyWith(
                      color: _message!.startsWith('Error') ? AppTheme.specRed : Colors.green,
                    ),
                  ),
                ],
