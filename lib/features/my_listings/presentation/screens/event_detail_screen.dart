...
import 'package:cajun_local/features/events/data/repositories/event_rsvps_repository.dart';
import 'package:cajun_local/core/theme/app_layout.dart';
import 'package:cajun_local/core/theme/theme.dart';

/// Full event details + attendee/RSVP analytics for listing owner or admin.
class EventDetailScreen extends StatelessWidget {
  const EventDetailScreen({super.key, required this.eventId, required this.listingId});
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: FutureBuilder<BusinessEvent?>(
        future: BusinessEventsRepository().getById(eventId),
        builder: (context, eventSnapshot) {
          ...
          if (event == null) {
            return Center(
              child: Text('Event not found', style: theme.textTheme.bodyLarge?.copyWith(color: nav)),
            );
          }
          return FutureBuilder<(Business? business, EventRsvpCounts counts)>(
            future: Future.wait([
                ...
                child: AppLayout.constrainSection(
                  ...
                  Column(
                    ...
                    children: [
                      ...
                      if (event.imageUrl != null && event.imageUrl!.trim().isNotEmpty) const SizedBox(height: 20),
                      Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            if (business != null) ...[
                              ...
                              Text(
                                business.name,
                                style: theme.textTheme.bodyMedium?.copyWith(color: sub, fontWeight: FontWeight.w500),
                              ),
                            ],
                            const SizedBox(height: 12),
                            Row(
                              children: [
                                Icon(Icons.calendar_today_rounded, size: 18, color: AppTheme.specGold),
                            ...
                            if (event.description != null && event.description!.trim().isNotEmpty) ...[
                              ...
                              Text(
                                event.description!,
                                style: theme.textTheme.bodyMedium?.copyWith(color: nav, height: 1.5),
                              ),
                            ],
                            const SizedBox(height: 8),
                            Container(
                              ...
                              child: Text(
                                ...
                                style: theme.textTheme.labelMedium?.copyWith(
                                  color: event.status == 'approved' ? Colors.green.shade800 : nav,
                      ...
                      Container(
                        ...
                        child: Column(
                          children: [
                            _CountRow(label: 'Going', count: counts.going, color: Colors.green),
                            const Divider(height: 24),
                            _CountRow(label: 'Interested', count: counts.interested, color: AppTheme.specGold),
                            const Divider(height: 24),
                            _CountRow(label: 'Not going', count: counts.notGoing, color: sub),
                            const SizedBox(height: 12),
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      ...
      children: [
        Text(
          label,
          style: theme.textTheme.bodyLarge?.copyWith(color: nav, fontWeight: FontWeight.w500),
        ),
        Text(
          '$count',
